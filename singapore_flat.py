...
st.header(':rainbow[Singapore Flat Resale Price Predictor]')
...
if selected == 'Home':
    ...
    with col2:
        st.image("https://i.gifer.com/zZI.gif", use_column_width=True)
        st.write("----")
        st.markdown("  ")
    ...
    with col1:
            st.image('https://cdn.pixabay.com/photo/2015/06/20/11/51/singapore-815721_1280.jpg', use_column_width=True)
    with col2:
            st.image('https://cdn.pixabay.com/photo/2017/07/31/06/20/singapore-2556628_1280.jpg', use_column_width=True)
...
if selected == "Get Prediction":
    ...
    st.markdown(title_text, unsafe_allow_html=True)
    st.markdown("<h5 style=color:orange>To Predict the Resale Price of a Flat, Please Fill the Information:",unsafe_allow_html=True)
    st.write('')
    ...
    if button:
        with st.spinner("Predicting..."):
            ...
            if not all([user_month,user_town,user_flat_type,user_flat_model,floor_area_sqm,price_per_sqm,year,block,
                ...
                user_data=np.array([[month, town, flat_type, block, flat_model,
                                        lease_commence_date, year, storey_start, storey_end,
                                        years_holding, floor_area_sqm_log,
                                        remaining_lease_log, price_per_sqm_log]])

                predict = reg_model.predict(user_data)
